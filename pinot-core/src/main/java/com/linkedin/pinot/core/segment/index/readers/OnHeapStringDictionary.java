...
public class OnHeapStringDictionary extends ImmutableDictionaryReader {
  ...
  public OnHeapStringDictionary(PinotDataBuffer dataBuffer, ColumnMetadata metadata) {
    ...
    for (int id = 0; id < cardinality; id++) {
      ...
      _idToStringMap[id] = value; // un-padded string
      _idToRawStringMap[id] = getStringFromBytes(bytes, false); // padded string
      _stringToIdMap.put(value, id);
    }

    // Close the data-buffer since on-heap dictionary has already been built and stored in-memory.
    try {
      close();
    } catch (IOException e) {
  ...
  public String getStringValue(int dictionaryId) {
    ...
    // Return padded string (compatible with off-heap StringDictionary).
